...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  The dispatcher and pre-drain recheck also read the authoritative backlog-hold predicate off-thread; a backlog-only captain hold keeps signal and stale rows on main, and an indeterminate hold read fails routing closed.
  Grant publication rechecks the candidate tasks under their task-control locks and retains every lock through publication; a hold that wins this boundary returns the wake to main without granting or consuming any rows.
...
- Consistency: `bin/fm-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/fm-lease.sh` is the command surface.
  Its durable captain-hold guard contract also governs branch steering and relaunch.
  The guards are wired into `fm-send.sh`, `fm-control.sh`, and `fm-teardown.sh` (overlap, lease-checked, with claim serialization retained through the mutation) and `fm-pr-merge.sh`, `fm-merge-local.sh`, and `fm-spawn.sh` (main-owned, branch refused; branch recovery through `fm-control` remains subject to the shared guards).
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  ...
  After an independently eligible wake has already been offered, a newly-arrived main-owned row for an unrelated task or a check-kind row does not revoke the offer: it is excluded from the eligible set, so whatever else is currently eligible still reaches the branch, and the main-owned row stays queued for main's own drain.
  If the original signal or stale trigger now names a decision-owned task, including one held after acceptance, the branch rejects its settlement before publishing a row grant so the watcher delivers that trigger to main, even when routine rows remain eligible or no worker status file exists.
...
The captain-hold cases in `tests/fm-pi-branch-extension.test.sh` cover backlog-only signal and stale routing, unreadable authority, post-acceptance holds, asynchronous mixed-task hold races, locked grant publication, main fallback preserving unread rows, and explicit release.
`tests/fm-send-inbox.test.sh` covers branch steering and relaunch refusal after main releases its lease, post-grant steering refusal, unreadable hold authority, main's stop path, and steering after explicit release.
