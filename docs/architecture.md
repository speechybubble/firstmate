...
A secondmate's recorded worktree is never probed for write activity, because it is a provisioned firstmate home whose own supervision keeps writing inside it whether or not the mate produces anything, so those writes cannot defer escalation.
A pane whose recorded endpoint holds no agent at all is not a wedge suspect: repeated alarms cannot recover an absent agent and would obscure alarms for live agents that may still recover.
